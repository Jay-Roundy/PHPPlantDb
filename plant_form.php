<?php
    ...
    if ($_POST['submit'])
    {
                ...
                !empty($seed) && !empty($area))
        {
            ...
            if (($image_error == 0) && ($image_size < GW_MAXFILESIZE))
            {
                //move the file to upload folder
                if(!empty(GW_UPLOADPATH . $image))
                {
                    echo('<p>Image already existed, used anyway ' . $target_image . '</p>');
                }
            ...
            {
                echo('<p>Image Not uploaded Check file size less than ' . 
                        GW_MAXFILESIZE . ' KB</p>');
            }
